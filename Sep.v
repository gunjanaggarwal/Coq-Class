...
specification of the [inc()] function:
<<
 { x = n }
 inc(x)
 { x = n + 1}
...
what the spec gives us directly: [x = n+1 ∧ y = m]. One approach is to
strengthen the spec of inc
clause

Separation logic says what you are allowed to asccess: both access control and
a specification of what the state is. The clause that says things other than x
aren't modified becomes implicit.
<<
 {x = n ∧ y = m }
 inc(x);
 inc(y);
...
reasoning about heaps and procedures. Separation logic is probably the most
popular.

Separation logic has a similar structure to Hoare logic, but the meaning of
{P}c{Q} is different. Semantically, ∀h1, ∀h2, if P(h1), then running c in heap
(h1 ⨄ h2) yields a heap h3 = h4 ⨄ h2 where Q(h4). Note that by + here we 
mean disjoint union.

Specifications in separation logic are defined by the following syntax
...
Module FunctionalSepIMP.
  ...
  *)
  ...
  (** Alas, reasoning isn't quite as simple as we might hope.  We have to
      ...
      but we must also so a lot of commuting and re-associating to discharge
      ...
      of thing.  Below, we'll see you an alternative technique based on 
      reflection. *)
