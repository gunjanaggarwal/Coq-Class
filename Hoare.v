...
Module FunctionalIMP(U : UNIVERSE).
  ...
  Definition Cmd(t:Type) :Type := heap -> option(heap*t).
  ...
  *)
  Lemma consequence_tc {t:Type} {c:Cmd t} {P1 P2 : hprop} {Q1 Q2:heap->t->hprop} : 
    ...
    {{ P2 }} c {{ Q2 }}.
  Proof.
    intros. unfold hoare_tc_triple in *. intros.
    specialize ( H h ).
   destruct (c h).
    - apply H0 in H2. apply H in H2. destruct p. 
      specialize (H1 _ _ _ H2 ). auto.
...
Compute run c.
(* Here we are proving that c will not return None. *)
Definition c_tc : 
  {{ top }}
  c
  {{ fun _ _ => top }}.
